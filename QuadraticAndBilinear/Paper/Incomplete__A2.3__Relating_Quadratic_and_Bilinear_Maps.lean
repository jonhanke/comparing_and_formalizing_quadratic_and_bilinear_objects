...
-- In A2.2__Quadratic_objects.lean

class IsQuadraticMap (Q : M → N) : Prop where
  /-- A quadratic map sends zero to zero. -/
  map_zero : Q 0 = 0
  /-- A quadratic map is homogeneous of degree 2. -/
  map_smul : ∀ (r : R) (m : M), Q (r • m) = (r * r) • Q m
  /-- The polar form of Q is additive in the first variable. -/
  polar_add_left : ∀ m₁ m₂ m₃ : M, polar Q (m₁ + m₂) m₃ = polar Q m₁ m₃ + polar Q m₂ m₃
  /-- The polar form of Q is additive in the second variable. -/
  polar_add_right : ∀ m₁ m₂ m₃ : M, polar Q m₁ (m₂ + m₃) = polar Q m₁ m₂ + polar Q m₁ m₃
...
-/
def polarBilinear (Q : M → N)
    -- Provide the specific proofs needed as direct arguments
    (h_zero : Q 0 = 0)
    (h_add_left : ∀ m₁ m₂ m₃, polar Q (m₁ + m₂) m₃ = polar Q m₁ m₃ + polar Q m₂ m₃)
    (h_add_right : ∀ m₁ m₂ m₃, polar Q m₁ (m₂ + m₃) = polar Q m₁ m₂ + polar Q m₁ m₃)
    : M →+ (M →+ N) :=
{ toFun := fun m₁ =>
    { toFun   := polar Q m₁,
      map_zero' := by
        -- Use the specific h_zero proof
        simp [polar, h_zero],
      map_add' := by
        intros m₂ m₃
        -- Use the specific h_add_right proof
        exact h_add_right m₁ m₂ m₃ },
  map_zero' := by
    ext m₂
    simp [polar, h_zero],
  map_add' := by
    ...
    -- Use the specific h_add_left proof
    exact h_add_left m₁ m₂ m₃ }
...
-- Helper lemmas for mapFromQuadraticForm

lemma mapFromQuadraticForm_zero {S : Type*} [Fintype S]
  (A : Matrix (Fin (Fintype.card S)) (Fin (Fintype.card S)) R) :
  mapFromQuadraticForm S A 0 = 0 := by
  simp [mapFromQuadraticForm, Matrix.mulVec_zero, dotProduct_zero]

lemma mapFromQuadraticForm_smul {S : Type*} [Fintype S]
  (A : Matrix (Fin (Fintype.card S)) (Fin (Fintype.card S)) R) (r : R) (m : (Fin (Fintype.card S) → R)) :
  mapFromQuadraticForm S A (r • m) = (r * r) • mapFromQuadraticForm S A m := by
  simp [mapFromQuadraticForm, Matrix.mulVec_smul, dotProduct_smul, mul_assoc]

lemma mapFromQuadraticForm_polar_add_left {S : Type*} [Fintype S]
  (A : Matrix (Fin (Fintype.card S)) (Fin (Fintype.card S)) R) (m₁ m₂ m₃ : (Fin (Fintype.card S) → R)) :
  polar (mapFromQuadraticForm S A) (m₁ + m₂) m₃ = polar (mapFromQuadraticForm S A) m₁ m₃ + polar (mapFromQuadraticForm S A) m₂ m₃ := by
  dsimp [polar, mapFromQuadraticForm]
  simp only [add_dotProduct, dotProduct_add, Matrix.mulVec_add]
  abel

lemma mapFromQuadraticForm_polar_add_right {S : Type*} [Fintype S]
  (A : Matrix (Fin (Fintype.card S)) (Fin (Fintype.card S)) R) (m₁ m₂ m₃ : (Fin (Fintype.card S) → R)) :
  polar (mapFromQuadraticForm S A) m₁ (m₂ + m₃) = polar (mapFromQuadraticForm S A) m₁ m₂ + polar (mapFromQuadraticForm S A) m₁ m₃ := by
  dsimp [polar, mapFromQuadraticForm]
  simp only [add_dotProduct, dotProduct_add, Matrix.mulVec_add]
...
theorem mapFromQuadraticForm_is_quadratic {S : Type*} [Fintype S]
  (A : Matrix (Fin (Fintype.card S)) (Fin (Fintype.card S)) R) :
  IsQuadraticMap (mapFromQuadraticForm S A) :=
{
  map_zero        := mapFromQuadraticForm_zero A,
  map_smul        := mapFromQuadraticForm_smul A,
  polar_add_left  := mapFromQuadraticForm_polar_add_left A,
  polar_add_right := mapFromQuadraticForm_polar_add_right A
}
...
/-
A2.3__Relating_Quadratic_and_Bilinear_Maps.lean
-/
